...
inline int rgb_conv(int r, int g, int b, int is_tcolor)
{
    return lookup[r] | (lookup[g] << 5) | (lookup[b] << 10) | (is_tcolor << 15);
}
...
int main(int argc, char *argv[])
{
    ...
    if (transparent_mode) {
        if(((header[0] & 0xFF) == 0x89) && img->trueColor) // header is PNG
            png_alpha_flag = 1;
        ...
        if (compact_mode == 1) {
            ...
            for (y = i = 0; y < h; ++y) {
                for (x = 0; x < w; ++x) {
                    ...
                    is_tcolor = png_alpha_flag ?
                        gdImageAlpha(img, color) >= ALPHA_THRESHOLD :
                        color == tcolor;
                    rgb = rgb_conv(
